...
# It will handle any event whose name ends with sunspot.
# It will inform SystemMetrics that the event should not be collected if the current user is an administrator.
# It will add the current user's name to the event payload
